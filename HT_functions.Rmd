...
raster_accumulator <- function(hs_points, d, n_theta_steps = 180, n_theta_cells = 180, n_rho_cells = 180, expected_n_line = 30) {
...
hough_line_detection <- function(points, xlim = c(-100, 100), num_theta_steps = 180, plot_hs = TRUE, n_theta_cells = 180, n_rho_cells = 180, expected_n_line = 30, tidy_up = TRUE, m_sep_threshold = pi/180, b_sep_threshold = NULL) {
  xmin <- xlim[1]; xmax <- xlim[2]
  ymin <- min(points[,2]); ymax <- max(points[,2])
  d <- sqrt((ymax - ymin)^2 + (xmax - xmin)^2)
  
  hs <- hough_space(cbind(points[,1], points[,2]), plot = plot_hs)
  accumulator <- raster_accumulator(hs, d)
  
  if (!tidy_up) {
    plot(points[,1], data_points[,2], ylim=c(ymin*1.5, ymax*1.5))
...
hough_line_detection(points = cbind(r_points[["x_coords"]], r_points[["y_coords"]]), xlim = c(xmin, xmax), expected_n_line = 10)
